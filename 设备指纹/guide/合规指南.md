...
| :--------:| ------------------------ | -|----------------------------------------------------------- | ------------------------------------------ | --------------------------------------------- |
| smsdk | 北京数美时代科技有限公司 |移动端（适用于Android/iOS/鸿蒙）| 必要个人信息：<br />1. 设备基本信息，包括设备品牌、设备制造商、设备型号、设备名称、设备系统类型及版本信息、设备基本设置、设备环境；<br />2. 设备标识信息，包括Android ID、OAID、IDFV（应用开发商标识符）、IDFA（广告标示符）；<br />3. 设备网络信息，包括网络的接入形式、周边WIFI列表、网络运营商信息、网络连接状态；<br />4. 设备应用信息，包括SDK宿主应用包名及版本号、运行中进程信息（仅获取当前进程名称）、传感器信息。<br /><br />可选个人信息：MAC地址、MediaDRM ID、软件列表、无线路由器标识（BSSID、SSID）及IP地址、粗略位置信息、精准位置信息、屏幕分辨率。 | 用于风险控制和反欺诈，以保障账户和交易安全 | https://www.ishumei.com/legal/cn/privacy.html |
| smsdk    | 北京数美时代科技有限公司  |          小程序端             | 设备品牌、设备型号、WiFi列表、网络类型、连接WiFi信息、ip信息、canvas指纹、系统版本号、微信客户端版本号、SDK版本号 |   用于风险控制和反欺诈，以保障账户和交易安全   | https://www.ishumei.com/legal/cn/privacy.html  |
| smsdk     | 北京数美时代科技有限公司  |          web端             | Canvas指纹、cookies信息、ua客户端浏览器信息 |   用于风险控制和反欺诈，以保障账户和交易安全   | https://www.ishumei.com/legal/cn/privacy.html  |
| smsdk | 北京数美时代科技有限公司 | Windows端 | 1. 设备信息，包括系统启动时间、品牌及型号、厂商、操作系统版本、基本设置、硬件信息、MAC地址、设备环境；<br />2. 日志信息，包括注册表信息；<br />3. 设备网络信息，包括网络的接入形式、网络连接状态；<br />4. 应用信息，包括SDK宿主应用包名及版本号、运行中进程信息、可疑exe/dll/sys全路径 | 用于风险控制和反欺诈，以保障账户和交易安全 | https://www.ishumei.com/legal/cn/privacy.html |
...
注意1：各 SDK 版本采集数据可能会有不同，具体采集项开关控制可以参照 《设备指纹SDK采集数据列表_android*.xlsx》文档中 默认是否采集 列显示 "是，可屏蔽 关键字：字段名" 即代表此字段可以不采集。采集列表文档随 sdk 一同发出。

注意2：Android smsdk 从 3.13.0 版本开始支持全字段采集可控
...
注意：iOS smsdk 从 3.13.0 版本开始支持全字段采集可控
...
... // 其它设置
// 配置不采集的字段
option.notCollect.push_back("a17"); // 表示不采集 MAC
// 启动SDK
BOOL isOk = sdk.Create(option);
```

注意：Windows smsdk 支持全字段采集可控
